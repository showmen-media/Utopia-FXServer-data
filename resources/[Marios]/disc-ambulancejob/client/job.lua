					...
					if data.current.value == 'revive' then
						...
						ESX.TriggerServerCallback('disc-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								ESX.TriggerServerCallback('disc-death:isPlayerDead', function(isPedDead)
									if isPedDead then
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastHospital ~= currentHospital or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
			...
			then
				TriggerEvent('disc-ambulancejob:hasExitedMarker', LastHospital, LastPart, LastPartNum)
				hasExited = true
			end

			HasAlreadyEnteredMarker, LastHospital, LastPart, LastPartNum = true, currentHospital, currentPart, currentPartNum

			TriggerEvent('disc-ambulancejob:hasEnteredMarker', currentHospital, currentPart, currentPartNum)
...
AddEventHandler('disc-ambulancejob:hasEnteredMarker', function(hospital, part, partNum)
...
AddEventHandler('disc-ambulancejob:hasExitedMarker', function(hospital, part, partNum)
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('disc-ambulancejob:storeNearbyVehicle', function(storeSuccess, foundNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('disc-ambulancejob:buyJobVehicle', function(bought)
					if bought then
...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy', {
		...
		TriggerServerEvent('disc-ambulancejob:giveItem', data.current.item, data.current.value)
	end, function(data, menu)
		menu.close()
	end)
end
...
RegisterNetEvent('disc-ambulancejob:heal')
AddEventHandler('disc-ambulancejob:heal', function(healType, quiet)
	local playerPed = PlayerPedId()
